...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 shadow-lg text-[11px]">
      ...
      <p className="text-amber-600 font-semibold">{d.minutes} min</p>
    </div>
  );
};

const TopDowntimeReasons = () => {
  const { topDowntimeReasons, downtimeLoading, datePreset, dateRange } = useDashboard() || {};
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm flex flex-col" style={{ height: 380 }}>
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <div>
          <h3 className="text-[13px] font-semibold text-slate-700 leading-tight">Top Downtime Reasons</h3>
      ...
      {downtimeLoading ? (
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={topDowntimeReasons} layout="vertical" margin={{ top: 5, right: 16, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 10, fill: "#64748b" }} axisLine={false} tickLine={false} />
            <YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 9, fill: "#64748b" }} axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} />
            <Bar dataKey="minutes" fill="#d97706" radius={[0, 3, 3, 0]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
...
export default TopDowntimeReasons;